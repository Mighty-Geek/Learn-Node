...
function loadPlaces(map) {
    ...
    _axios2.default.get('api/stores/near?lat=' + lat + '&lng=' + lng).then(function (res) {
        ...
        // when someone clicks on marker show up the details
...
function typeAhead(search) {
    ...
    searchInput.on('input', function () {
        ...
        _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
            ...
            // tell them no results for query found
    ...
    searchInput.on('keyup', function (e) {
        // if they are not pressing up/down/enter, who cares!!
        if (![13, 38, 40].includes(e.keyCode)) {
            return;
        }
        var activeClass = 'search__result--active';
        var curr = search.querySelector('.' + activeClass);
        var items = search.querySelectorAll('.search__result');
        var next = void 0;
        if (e.keyCode === 40 && curr) {
            next = curr.nextElementSibling || items[0];
        } else if (e.keyCode === 40) {
            next = items[0];
        } else if (e.keyCode === 38 && curr) {
            next = curr.previousElementSibling || items[items.length - 1];
